use crate::htmlv::get_tera;
use axum::{
    Form, Router,
    extract::Query,
    http::StatusCode,
    response::{Html, IntoResponse, Redirect},
    routing::{get, post},
};
use axum_messages::{Message, Messages};
use serde::Deserialize;
use tera::Context;
use tracing;

use crate::auth::users::{AuthSession, Credentials};

fn render_login_template(
    messages: Vec<Message>,
    next: Option<String>,
) -> Result<String, tera::Error> {
    println!(
        "Rendering login template with messages: {:?} and next: {:?}",
        messages, next
    );
    let tera = get_tera();
    let mut context = Context::new();
    let messages_as_strs: Vec<String> = messages.into_iter().map(|m| m.message).collect();
    context.insert("messages", &messages_as_strs);
    context.insert("next", &next);
    tera.render("login_template.html", &context)
}

// This allows us to extract the "next" field from the query string,
// to redirect after log in.
#[derive(Debug, Deserialize)]
pub struct NextUrl {
    next: Option<String>,
}

pub fn router() -> Router<()> {
    Router::new()
        .route("/login", post(self::post::login))
        .route("/login", get(self::get::login))
        .route("/logout", get(self::get::logout))
}

mod post {
    use super::*;

    pub async fn login(
        mut auth_session: AuthSession,
        messages: Messages,
        Form(creds): Form<Credentials>,
    ) -> impl IntoResponse {
        let user = match auth_session.authenticate(creds.clone()).await {
            Ok(Some(user)) => user,
            Ok(None) => {
                messages.error("Invalid credentials");

                let mut login_url = "/login".to_string();
                if let Some(next) = creds.next {
                    login_url = format!("{login_url}?next={next}");
                };

                return Redirect::to(&login_url).into_response();
            }
            Err(_) => return StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        };

        if auth_session.login(&user).await.is_err() {
            return StatusCode::INTERNAL_SERVER_ERROR.into_response();
        }

        messages.success(format!("Successfully logged in as {}", user.username));

        if let Some(ref next) = creds.next {
            Redirect::to(next)
        } else {
            Redirect::to("/protected/private")
        }
        .into_response()
    }
}

mod get {
    use super::*;

    pub async fn login(
        messages: Messages,
        Query(NextUrl { next }): Query<NextUrl>,
    ) -> Html<String> {
        tracing::info!("LOGGIN {:?}", next);
        let rendered = render_login_template(messages.into_iter().collect(), next)
            .expect("template render failed");

        Html(rendered)
    }

    pub async fn logout(mut auth_session: AuthSession) -> impl IntoResponse {
        match auth_session.logout().await {
            Ok(_) => Redirect::to("/login").into_response(),
            Err(_) => StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        }
    }
}
